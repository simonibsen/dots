...
# Set up prompt with bash-git-prompt
if [ -f "$(brew --prefix)/opt/bash-git-prompt/share/gitprompt.sh" ]; then
    __GIT_PROMPT_DIR="$(brew --prefix)/opt/bash-git-prompt/share"
    GIT_PROMPT_ONLY_IN_REPO=1  # Only show git prompt when in a git repo

    # Inject AWS role before git prompt
    GIT_PROMPT_START="\[\033[33m\]\$(get_aws_role)\[\033[00m\] \[\033[01;32m\]\u@\h\[\033[00m\]:\[\033[01;34m\]\w\[\033[00m\]"
...
alias tree='tree -Csu'
alias claude='caffeinate -i claude'  # keep Mac awake while Claude runs
...
mdclean() {
    pbpaste | awk '
      ...
      /^$/ { flush(); closetbl(); print; pb=1; hd=0; inbul=0; next }
      ...
      /^ *- / { flush(); sub(/^ +/, ""); buf=$0; pb=0; hd=1; inbul=1; next }

      # Heading: short line after blank line
      pb && length($0)<80 {
        flush()
        sub(/^ +/, "")
        buf="## "$0; pb=0; hd=1; inbul=0; next
      }
